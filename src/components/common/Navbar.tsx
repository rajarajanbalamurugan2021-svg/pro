              ...
              <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl z-50 p-4 space-y-3">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">{unreadCount} unread • {safeNotifications.length} total</p>
                  </div>
                  <div className="flex items-center gap-1.5">
                    {unreadCount > 0 && onMarkAllNotificationsRead && (
                      <button
                        onClick={onMarkAllNotificationsRead}
                        className="px-2 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/60 hover:bg-blue-100 text-blue-600 dark:text-blue-400 text-[11px] font-semibold transition flex items-center gap-1 cursor-pointer"
                    ...
                    {safeNotifications.length > 0 && onClearAllNotifications && (
                      <button
                        onClick={onClearAllNotifications}
                        className="p-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/50 text-slate-400 transition cursor-pointer"
                ...
                <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
                  {safeNotifications.length === 0 ? (
                    <div className="text-center py-8 space-y-2">
                      <Bell className="h-8 w-8 text-slate-300 dark:text-slate-600 mx-auto opacity-50" />
                    ...
                    safeNotifications.map((n) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (n.linkModule && onNavigateModule) {
                        ...
                        className={`p-3 rounded-xl border text-xs cursor-pointer transition relative group ${
                          n.read
                            ? 'bg-slate-50/60 dark:bg-slate-800/30 border-slate-100 dark:border-slate-800/80 text-slate-500 dark:text-slate-400 hover:bg-slate-100/80 dark:hover:bg-slate-800/60'
                            : 'bg-blue-50/70 dark:bg-blue-950/40 border-blue-200/80 dark:border-blue-900/60 text-slate-800 dark:text-slate-200 font-medium shadow-2xs hover:border-blue-300'
                        }`}
                      >
                        <div className="flex items-center gap-2 mb-1">
                          {n.type === 'info' && <Info className="h-3.5 w-3.5 text-blue-500 shrink-0" />}
                          {n.type === 'success' && <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 shrink-0" />}
                          {n.type === 'warning' && <AlertTriangle className="h-3.5 w-3.5 text-amber-500 shrink-0" />}
                          <span className="font-bold text-slate-900 dark:text-white line-clamp-1 flex-1">{n.title}</span>
                          {!n.read && <span className="h-2 w-2 rounded-full bg-blue-500 shrink-0"></span>}
                          <span className="text-[10px] text-slate-400 shrink-0">{n.timestamp}</span>
                        </div>
                        <p className="leading-relaxed text-slate-600 dark:text-slate-300">{n.message}</p>
                        {n.linkModule && (
                          <div className="mt-2 flex items-center justify-end text-[10px] font-bold text-blue-600 dark:text-blue-400 gap-1 opacity-80 group-hover:opacity-100 transition">
                            <span>Open {n.linkModule.replace('_', ' ')}</span>
